    ...
    <div className='w-full min-h-screen flex justify-center items-center p-4'>
        ...
        <div className='w-full max-w-[500px] min-h-[650px] flex items-center justify-center flex-col  overflow-hidden '>
            <div className='w-[60%] h-[150px] md:h-[200px] flex items-center justify-center mt-4'>
                <img src={waitimg} draggable={false} alt="Waiting" className='w-full h-full object-cover'/>
            </div>
            ...
            <div className='w-full py-1 flex justify-center items-center'>
                <div className='w-[94%] p-2 flex gap-3 items-center justify-between flex-row rounded-lg shadow-lg'>
                    <div className='h-20 w-20 md:h-[110px] md:w-[110px] shrink-0 flex items-center justify-center'>
                        <img src={bookingData?.listing?.image1} alt="" className='w-full h-full object-cover rounded-lg'/>
                    </div>
                    ...
                    <div className='flex flex-col grow m-0 p-0 text-left overflow-hidden'>
                        ...
                        <div className='text-xs md:text-[16px] text-black flex flex-row items-center gap-1'>
                            <IoLocationOutline /> {`${bookingData?.listing?.landmark} / ${bookingData?.listing?.city}`}
                        </div>
                        <p className='text-xs md:text-[14px] text-black mt-1'>
                            Check-in: { bookingData?.checkIn?.split('T')[0] } <br />
                            Check-out: { bookingData?.checkOut?.split('T')[0] }
                        </p>
                        
                        
                        <div className='flex w-full justify-between items-center mt-0'>
                            <p className='text-[10px] md:text-[14px] font-semibold'>
                                Price per night: { bookingData?.listing?.rent }
                            </p>
                            <p className='text-[12px] md:text-[16px] text-red-600 font-bold'>
                                To pay: { bookingData?.totalRent }
                            </p>
                        </div>
            ...
            <div className='w-full py-2 px-4'>
                ...
                <div className='flex flex-row items-center gap-2 text-xs md:text-sm'> <CiUser /> {`${bookingData?.host?.name} / ${bookingData?.host?.email}`}  </div>
                <div className='flex flex-row items-center gap-2 text-xs md:text-sm mt-1'> <CiPhone /> {bookingData?.host?.phone} </div>
            </div>

            <div className='w-full h-[70px] p-2'>
                <button onClick={() => navigate('/')} className='bg-red-500 rounded-xl text-lg font-bold cursor-pointer transition-all hover:bg-red-600 text-white w-full h-full shadow-md active:scale-95'>
                    Back to home 
                </button>
            </div>
        </div>
